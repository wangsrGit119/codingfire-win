...
namespace CodingFire.Core
{
    ...
    public static class L10n
    {
        ...
        private static readonly Dictionary<string, string[]> Table = new Dictionary<string, string[]>(StringComparer.Ordinal)
        {
            ...
            { "console.openFailed",  new[] { "Could not open the console.", "无法打开控制台。", "コンソールを開けません。", "콘솔을 열 수 없습니다." } },
            ...
            { "stats.overview",  new[] { "Usage overview", "用量概览", "使用量の概要", "사용량 개요" } },
            { "stats.last7",      new[] { "Last 7 days", "近 7 天", "過去 7 日間", "최근 7일" } },
            { "stats.last30",     new[] { "Last 30 days", "近 30 天", "過去 30 日間", "최근 30일" } },
            { "stats.activeDays", new[] { "Active days", "活跃天数", "アクティブ日数", "활성 일수" } },
            { "stats.storedHistory", new[] { "Stored history", "已保存历史", "保存履歴", "저장된 기록" } },
            ...
            { "about.powered", new[] { ".NET Framework 4.x (CLR 4.0) · no installer · runs on Windows 8 and later",
                                       ".NET Framework 4.x（CLR 4.0）· 免安装 · Windows 8 及以上可直接运行",
                                       ".NET Framework 4.x（CLR 4.0）· インストーラ不要 · Windows 8 以降で動作",
                                       ".NET Framework 4.x(CLR 4.0) · 설치 불필요 · Windows 8 이상에서 실행" } },
